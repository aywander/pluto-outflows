...
static void
bst_trav_refresh (struct bst_traverser *trav)
{
  assert (trav != NULL);
...
static void
bst_copy_error_recovery (struct bst_node **stack, int height,
          ...
          if (height >= 2 * (BST_MAX_HEIGHT + 1))
            {
              ...
              bst_copy_error_recovery (stack, height, new, destroy);
...
static void
bst_tree_to_vine (struct bst_table *tree)
{
  struct bst_node *q, *p;
...
/* Performs a compression transformation |count| times,
   starting at |root|. */
static void
bst_compress (struct bst_node *root, unsigned long count)
{
  assert (root != NULL);
...
/* Converts |tree|, which must be in the shape of a vine, into a balanced
   tree. */
static void
bst_vine_to_tree (struct bst_table *tree)
{
  ...
  bst_compress ((struct bst_node *) &tree->bst_root, leaves);

  vine = tree->bst_count - leaves;
  height = 1 + (leaves > 0);
  while (vine > 1)
    {
      bst_compress ((struct bst_node *) &tree->bst_root, vine / 2);
      vine /= 2;
      height++;
    }
...
bst_balance (struct bst_table *tree)
{
  ...
  bst_tree_to_vine (tree);
  bst_vine_to_tree (tree);
  tree->bst_generation++;
}
