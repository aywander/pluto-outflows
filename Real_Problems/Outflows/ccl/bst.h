...
struct bst_table
  {
    ...
    unsigned int  bst_count;                   /* Number of items in tree. */
...
struct bst_traverser
  {
    ...
    unsigned int  bst_height;                  /* Number of nodes in |bst_parent|. */
    unsigned long bst_generation;       /* Generation number. */
  };
...
#define bst_count(table) ((unsigned int) (table)->bst_count)
