...
double lhonergrad2(double ***q,int i,int j,int k, int m_IDIR, int m_JDIR, int m_KDIR);
...
*/
 ...
 #endif
  double ***q, ***grad;
  double ***qtr1,***qtr2,grad2q,grad2qtr1,grad2qtr2;
  int dsizeI,dsizeJ,dsizeK;
  RBox  Ubox, Gbox;
...
//----DM: 9Mar15: define arrays for tracers, if defined for two tracers (default for Jet set up----  
   if (NTRACER > 1 ){
	qtr1 = ArrayBox(Ubox.kb, Ubox.ke, Ubox.jb, Ubox.je, Ubox.ib, Ubox.ie);
   	qtr2 = ArrayBox(Ubox.kb, Ubox.ke, Ubox.jb, Ubox.je, Ubox.ib, Ubox.ie);
   	qtr1 = UU[TRC];
   	qtr2 = UU[TRC+1];
	}
  #else
...
/* ----------------------------------------------------------------
  ...
  BOX_LOOP(&Gbox, k, j, i){
    ...
    dsizeI=dsizeJ=dsizeK=1;
    D_EXPAND(dsizeI=m_domain.size(IDIR);, dsizeJ=m_domain.size(JDIR);,dsizeK=m_domain.size(KDIR););


    grad2q=lhonergrad2(q,i,j,k,dsizeI,dsizeJ,dsizeK);

    grad[k][j][i] = sqrt(grad2q);
...
//-----DM: 9Mar15: Generic function to compute and return square of lhoner gradient of any variable----
double lhonergrad2(double ***q,int i,int j, int k, int m_IDIR, int m_JDIR, int m_KDIR)
{
  double  dqx_p, dqx_m, dqx, d2qx, den_x;
  double  dqy_p, dqy_m, dqy, d2qy, den_y;
  double  dqz_p, dqz_m, dqz, d2qz, den_z;
  double gr1, gr2, eps = 0.01;
  ...
  /* --------------------------------------------------------------
    ...
    D_EXPAND(if (i == m_IDIR-1) dqx_p = dqx_m;  ,
             if (j == m_JDIR-1) dqy_p = dqy_m;  ,
             if (k == m_KDIR-1) dqz_p = dqz_m;)
