 ...
 **************************************************************** */
{
  ...
  for (j = jbeg; j <= jend; j++){ g_j = j;
    ...
    if (err != 0) {
       ...
       for (i = ibeg; i <= iend; i++){ 
       if (flag[i] != 0) {
//----DM: 9Mar15: fix negative pressure, density, energy. Currently for 3D.----//
      if(COMPONENTS == 3){
	...
	V[PRS][k][j][i]=prsfix;
	scrh   = 1.0/(U[ENG][k][j][i] + prsfix); 
    	EXPAND( V[VX1][k][j][i] = U[MX1][k][j][i]*scrh; ,
           	V[VX2][k][j][i] = U[MX2][k][j][i]*scrh; ,
           	V[VX3][k][j][i] = U[MX3][k][j][i]*scrh;)
