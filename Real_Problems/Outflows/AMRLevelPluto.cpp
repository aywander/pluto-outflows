                           ...
                           int                  a_refRatio)
{
  ...
  m_dx = m_domainLength / (a_problemDomain.domainBox().bigEnd(1)-a_problemDomain.domainBox().smallEnd(1)+1.);
#else
...
void AMRLevelPluto::tagCells(IntVectSet& a_tags) 
{
  ...
  for (dit.begin(); dit.ok(); ++dit){
    ...
    //m_patchPluto->computeRefGradient(gradFab, UFab, b); 
    m_patchPluto->computeRefGradient(gradFab, UFab, b, m_level); 
    /* -- AYW */
...
void AMRLevelPluto::writeCheckpointLevel(HDF5Handle& a_handle) const
{
  ...
  for(;dit.ok(); ++dit){
       ...
       for(BoxIterator bit(curBox); bit.ok(); ++bit) {
        const IntVect& iv = bit();
        Real volume_1 = 1./(g_domBeg[0]+(iv[0]+0.5)*m_dx);
        for(int ivar = 0; ivar < curU.nComp(); ivar++) {
          curU(iv, ivar) *= volume_1;
        }
       }
  }
  ...
  for(;dit.ok(); ++dit){
       ...
       for(BoxIterator bit(curBox); bit.ok(); ++bit) {
        const IntVect& iv = bit();
 #if (LOGR == NO)
        Real x1l = g_domBeg[0]+iv[0]*m_dx;
        Real x1r = x1l + m_dx;
        Real volume_1 = 3./(x1r*x1r*x1r-x1l*x1l*x1l)*m_dx;
 #else
        Real x1l = iv[0]*m_dx;
        Real x1r = x1l + m_dx;
        Real volume_1 = 3./(exp(3.*x1r)-exp(3.*x1l))*m_dx;
        volume_1 /= g_domBeg[IDIR]*g_domBeg[IDIR]*g_domBeg[IDIR];
 #endif
#if CH_SPACEDIM > 1
        Real x2l = g_domBeg[1]+iv[1]*m_dx;
        Real x2r = x2l + m_dx;
        volume_1 *= m_dx/(cos(x2l)-cos(x2r));
#endif
        for(int ivar = 0; ivar < curU.nComp(); ivar++) {
          curU(iv, ivar) *= volume_1;
        }
...
void AMRLevelPluto::readCheckpointLevel(HDF5Handle& a_handle)
{
  ...
  ch_max = header.m_real["ch"];

  if (s_verbosity >= 2)
  {
    pout() << "read ch max = " << ch_max << endl;
  }
 #endif
  ...
  for(;dit.ok(); ++dit){
       ...
       for(BoxIterator bit(curBox); bit.ok(); ++bit) {
        const IntVect& iv = bit();
        Real volume = g_domBeg[0]+(iv[0]+0.5)*m_dx;
        for(int ivar = 0; ivar < curU.nComp(); ivar++) {
         curU(iv, ivar) *= volume;
        }
       }
  ...
  for(;dit.ok(); ++dit){
       ...
       for(BoxIterator bit(curBox); bit.ok(); ++bit) {
        ...
        Real x1l = g_domBeg[0]+iv[0]*m_dx;
        Real x1r = x1l + m_dx;
        Real volume = (x1r*x1r*x1r-x1l*x1l*x1l)/3./m_dx;
 #else
        Real x1l = iv[0]*m_dx;
        Real x1r = x1l + m_dx;
        Real volume = (exp(3.*x1r)-exp(3.*x1l))/3./m_dx;
        volume *= g_domBeg[IDIR]*g_domBeg[IDIR]*g_domBeg[IDIR];
 #endif
#if CH_SPACEDIM > 1
        Real x2l = g_domBeg[1]+iv[1]*m_dx;
        Real x2r = x2l + m_dx;
        volume *= (cos(x2l)-cos(x2r))/m_dx;
#endif
        for(int ivar = 0; ivar < curU.nComp(); ivar++) {
         curU(iv, ivar) *= volume;
        }
...
void AMRLevelPluto::DefineExpressions(HDF5HeaderData& a_expressions) const
{
 ...
 // Grid/geometry expressions
  ...
  #if DIMENSIONS == 2
   sprintf (str_expr,"coords(Mesh)[1]+nodal_constant(Mesh,%12.6e)",g_domBeg[JDIR]);
   ...
   sprintf (str_expr,"coords(Mesh)[1]+nodal_constant(Mesh,%12.6e)",g_domBeg[JDIR]);
   a_expressions.m_string["scalar Y"] = str_expr;
   sprintf (str_expr,"coords(Mesh)[2]+nodal_constant(Mesh,%12.6e)",g_domBeg[KDIR]);
